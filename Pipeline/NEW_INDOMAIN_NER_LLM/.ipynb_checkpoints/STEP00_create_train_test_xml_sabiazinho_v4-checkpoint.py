...
outputdir=pipeline+"NEW_INDOMAIN_NER_LLM/"+resultsdir
print(f"output dir = {outputdir}") # new AT may 2025

os.makedirs(outputdir, exist_ok=True)


# # Split the train set in 2 parts: TRAIN AND TEST
# 
# `TODO: Não fazer split aqui e usar split inicial do DATASET`
# 
# TO BE IMPLEMENTED
...
test_percentage = 30 # use to configure
...
# DEBUG
##  para ter termo de comparação

path_data = pipeline+"NEW_INDOMAIN_NER_LLM/OUTPUT_RESULTS/"
filename_input = "results_top10_adjst.csv" 
aux = path_data+filename_input
print(f"Reading examples from {aux}")
em_word_tag_df = (
    pd.read_csv(aux, sep='\t')[['WORD', 'MAPPER10', 'line']]
    .rename(columns={'MAPPER10': 'tag','WORD': 'word'})
)
...
outputdir =pipeline+"NEW_INDOMAIN_NER_LLM/"+resultsdir
outputdir_maritalk =pipeline+"NEW_INDOMAIN_NER_LLM/"+resultsdir+"maritalk"
...
################################################################################################
OUTPUT_FILE = f'{outputdir}XML_2PerClass_10class_sabiazinho3_{dataset}_pt_512_20250519.txt'
